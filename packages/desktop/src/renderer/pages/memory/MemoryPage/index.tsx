...
import { MemoryInboxTab } from './MemoryInboxTab';
import { MemoryPageShell } from './MemoryPageShell';
import { MemoryScopeTabs } from './MemoryScopeTabs';
import { isMemoryPageTab, memoryPageTabToScope, type MemoryPageTab } from './memoryPageTab';
...
const MemoryPage: React.FC = () => {
  ...
  const pageTab: MemoryPageTab = isMemoryPageTab(tabParam) ? tabParam : 'personal';
  const scope: MemoryScope | null = memoryPageTabToScope(pageTab);
  const precipitationSummary = usePrecipitationSummary(ccbActive);
  const pendingCount = precipitationSummary?.pendingCount ?? 0;
  ...
  const listKey = ccbActive && scope ? `ccb-memory.list.${scope}` : null;
  const {
    data: files,
    error: listError,
    ...
    listKey,
    () => invokeWithTimeout(ipcBridge.ccbPersonalMemoryService.listFiles.invoke({ scope: scope! })),
    {
  ...
  const contentKey = ccbActive && selectedRel && scope ? `ccb-memory.file.${selectedRel}` : null;
  const {
    data: fileContent,
    error: readError,
  ...
  const scopeEmpty = scope && !listBusy && !listError && fileList.length === 0;
  ...
  const headerActions =
    pageTab === 'inbox' ? (
      <Button
        size='small'
        icon={<IconRefresh />}
        onClick={() => void globalMutate('ccb-precipitation.pending')}
      >
  ...
  return (
    <MemoryPageShell
      ...
      toolbar={<MemoryScopeTabs tab={pageTab} pendingCount={pendingCount} onChange={setTab} />}
      banner={
        listError || readError ? (
          <Alert
            type='error'
    ...
    >
      {pageTab === 'inbox' ? (
        <MemoryInboxTab highlightId={highlightId} conversationId={conversationFilter} />
      ) : scopeEmpty ? (
